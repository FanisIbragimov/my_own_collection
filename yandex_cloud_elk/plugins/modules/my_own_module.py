...
def run_module():
    ...
    if file_exist(module.params["path"]):
      ...
      result['changed'] = True
      result['original_message'] = 'File {path} succesully created'.format(path = module.params['path'])
      result['message'] = 'file created'
    else:
      result["original message"] = "File {path} already exist".format(path=module.params["path"])
